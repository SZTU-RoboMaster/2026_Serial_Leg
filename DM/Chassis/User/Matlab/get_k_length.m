function K = get_k_length(leg_length,dt) % s
   ...
   IM1=0.415329361;
    ...
    % x / x_dot 不能在 6 状态 dlqr 里直接设成 0，否则很容易让 Riccati 方程无稳定解
    % 这里先给极小正权重，等 K 能算出来后，再在实车上把 x 通道从控制输出里关掉
    % 如果你要做严格的四状态隔离版，建议另起一个 4 状态模型，而不是在这里硬置零
%         Q_cost=diag([1000 0.5 100 100 3000 6]);
%         R_cost=diag([10, 0.1]);

      Q = diag([1000  0.5  100  100  20000  100]);
    R = [10  0;
        0  0.1];                         % t tp
