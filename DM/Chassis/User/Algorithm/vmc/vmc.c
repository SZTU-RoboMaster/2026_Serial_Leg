...
void vmc_phi_update(Leg *leg_L, Leg *leg_R) {
    ...
    // 待验证
    leg_L->vmc.forward_kinematics.fk_phi.phi1_dot = (get_joint_motors() + 0)->angular_vel;
    leg_L->vmc.forward_kinematics.fk_phi.phi4_dot = (get_joint_motors() + 1)->angular_vel;
    leg_R->vmc.forward_kinematics.fk_phi.phi1_dot = -(get_joint_motors() + 2)->angular_vel;
    leg_R->vmc.forward_kinematics.fk_phi.phi4_dot = -(get_joint_motors() + 3)->angular_vel;
...
static void forward_kinematics(Leg *leg_L, Leg *leg_R, ChassisPhysicalConfig *physical_config) {
    ...
    leg_L->vmc.forward_kinematics.fk_point_coordinates.d_x =
            physical_config->l5 + physical_config->l4 * cosf(leg_L->vmc.forward_kinematics.fk_phi.phi4);
    leg_L->vmc.forward_kinematics.fk_point_coordinates.d_y =
            physical_config->l4 * sinf(leg_L->vmc.forward_kinematics.fk_phi.phi4);

    float L_A0 = 2.0f * physical_config->l2 * (leg_L->vmc.forward_kinematics.fk_point_coordinates.d_x -
                                               leg_L->vmc.forward_kinematics.fk_point_coordinates.b_x);
    float L_B0 = 2.0f * physical_config->l2 * (leg_L->vmc.forward_kinematics.fk_point_coordinates.d_y -
                                               leg_L->vmc.forward_kinematics.fk_point_coordinates.b_y);
    ...
    leg_L->vmc.forward_kinematics.fk_phi.dd_phi0 =
            (leg_L->vmc.forward_kinematics.fk_phi.d_phi0 - leg_L->vmc.forward_kinematics.fk_phi.last_d_phi0)
            / (CHASSIS_PERIOD * 0.001f);
    ...
    leg_R->vmc.forward_kinematics.fk_point_coordinates.d_x =
            physical_config->l5 + physical_config->l4 * cosf(leg_R->vmc.forward_kinematics.fk_phi.phi4);
    leg_R->vmc.forward_kinematics.fk_point_coordinates.d_y =
            physical_config->l4 * sinf(leg_R->vmc.forward_kinematics.fk_phi.phi4);

    float R_A0 = 2.0f * physical_config->l2 * (leg_R->vmc.forward_kinematics.fk_point_coordinates.d_x -
                                               leg_R->vmc.forward_kinematics.fk_point_coordinates.b_x);
    float R_B0 = 2.0f * physical_config->l2 * (leg_R->vmc.forward_kinematics.fk_point_coordinates.d_y -
                                               leg_R->vmc.forward_kinematics.fk_point_coordinates.b_y);
    ...
    leg_R->vmc.forward_kinematics.fk_phi.dd_phi0 =
            (leg_R->vmc.forward_kinematics.fk_phi.d_phi0 - leg_R->vmc.forward_kinematics.fk_phi.last_d_phi0) /
            (CHASSIS_PERIOD * 0.001f);
...
//    forward_kinematics(&chassis.leg_R, &chassis_physical_config);
    ...
    USART_Vofa_Justfloat_Transmit(chassis.leg_L.vmc.forward_kinematics.fk_L0.L0,
